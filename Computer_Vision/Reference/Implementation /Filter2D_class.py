...
class Filter2D(object):
    ...
    def __init__(self, data, method, **keyval):
        ...
        for k, v in keyval.iteritems():
            ...
            if type(a) != type(v):
                raise Exception(
                    "Cannot change the type of an attribute. %s must be a %s not a %s." % (k, type(a), type(v)))
    ...
    def checkmethod(self, method):
        ...
        for m in self.methods:
            if m.startswith(method):
                return m
        raise Exception(
            "Unknown method %s given for smoothing. Available methods are: %s" % (method, str(self.methods)))
